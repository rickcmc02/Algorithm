...
dq_left = deque(sys.stdin.readline().rstrip())
dq_right = deque()
m = int(sys.stdin.readline())

for i in range(m):
    cmd = sys.stdin.readline().rstrip()
    if cmd[0] == "L" and dq_left:
        dq_right.appendleft(dq_left.pop())
    elif cmd[0] == "D" and dq_right:
        dq_left.append(dq_right.popleft())
    elif cmd[0] == "B" and dq_left:
        dq_left.pop()
    elif cmd[0] == "P":
        dq_left.append(cmd[2])

print(''.join(dq_left + dq_right))


'''
# 첫번째 시도 - slicing이 시간복잡도 O(N)이라 시간초과

for i in range(m):
    cmd = sys.stdin.readline().strip()
    if cmd[0] == "L" and cur_loc:
        cur_loc -= 1
    elif cmd[0] == "D" and cur_loc < len(s):
        cur_loc += 1
    elif cmd[0] == "B" and cur_loc:
        s = s[:cur_loc-1] + s[cur_loc:]
        cur_loc -= 1
    elif cmd[0] == "P":
        s = s[:cur_loc] + cmd[2] + s[cur_loc:]
        cur_loc += 1
'''
